    ...
    <div className="patient-list px-4 pl-8 py-2 ">
      ...
      <div className="h-[600px] mt-4  py-2 px-4 rounded-md bg-white relative">
        <div className="flex justify-between  px-2 py-1 pr-10">
          <div className="flex gap-5 items-center">
            ...
            <div className="search relative">
              <input
                ...
                className="rounded-lg h-10 w-72 bg-gray-100 px-2  pb-1 pr-7"
              />
              <i
                onClick={handleSeachIconClick}
                className="fa-solid fa-magnifying-glass absolute right-3 bottom-3 text-gray-500 cursor-pointer"
              ></i>
              {showDetails && (
                <motion.div
                ...
                >
                  {displaySearchResult?.map((patient) => {
                    return (
                      <div
                        key={patient?._id}
                        className={` min-h-[450px] relative pb-12 `}
                      >
                        <div className="flex justify-between">
                          <h2 className="text-xl opacity-80 pointer-events-none font-semibold p-2">
                            {patient?.patientName[0].toUpperCase() +
                              patient?.patientName.slice(1)}
                          </h2>
                          <div onClick={handleCancelShowDetail} className="cut">
                          ...
                          <div>
                            <div className="p-1 px-10 w-full center">
                              <img
                                src={patient?.image}
                                alt="profile picture"
                                className=" h-32 w-32 hover:scale-[1.01] hover: transition-all duration-300 rounded"
                              />
                            </div>
                            <p className="mt-1 p-1 px-10 rounded-md animate w-96 bg-blue-300 hover:bg-gray-400 font-medium  hover:text-white flex gap-8">
                              <div className="w-14">Gender:</div>{" "}
                              <div>
                            ...
                            <p className="mt-1 p-1 px-10 rounded-md animate w-96 bg-blue-300 hover:bg-gray-400 font-medium  hover:text-white flex gap-8">
                              <div className="w-14">Contact: </div>
                              <div> {patient?.contact}</div>
                            </p>
                            <p className="mt-1 p-1 px-10 rounded-md animate w-96 bg-blue-300 hover:bg-gray-400 font-medium  hover:text-white flex gap-8">
                              <div className="w-14">Problem:</div>{" "}
                              <div>{patient?.complaint}</div>
                            </p>
                            <p
                              className={`mt-1 p-1 px-10 rounded-md animate w-96 bg-blue-300 font-medium  hover:text-white flex gap-8 ${
                        ...
                        <div className="absolute bottom-0 mt-5 w-full  flex justify-between">
                          <div className="flex gap-4 ml-2 ">
                            ...
                            <i
                              ...
                              className="fa-solid fa-trash-can text-red-600 delete hover:text-red-900"
                            ></i>
                          </div>
                          <div className="text-sm flex gap-2">
                            Showing {displaySearch + 1}th of{" "}
                            {searchResults.length} found
                            <button
                              onClick={prevSearchResult}
                              className="size-5 rounded bg-blue-200 hover:bg-blue-300 "
                            >
                              <i className="fa-solid fa-chevron-left"></i>
                            </button>
                            <span className="size-5 rounded bg-blue-200 hover:bg-blue-300 center">
                              {displaySearch + 1}
                            </span>
                            <button
            ...
            <Link to="/home/prescription">
              <div className="bg-gray-100 hover:bg-gray-200 animate text-gray-800 center size-8 rounded-full cursor-pointer">
                <i className="fa-solid fa-plus"></i>
              </div>
            </Link>
            <div
              onClick={handleRefresh}
              className="bg-gray-100 hover:bg-gray-200 animate text-gray-800 center size-8 rounded-full cursor-pointer"
            >
              <i className="fa-solid fa-rotate"></i>
            </div>
            <div
              onClick={handleShowAllPatients}
              className={`animate center size-8 rounded-full cursor-pointer font-medium ${
                activeFilter === "all"
            ...
            <div className="">
              <input
                ...
                className={`ml-5 px-5 border rounded-md focus:outline-none hover:border-blue-500 focus:border-blue-500`}
              />
              <input
                type="date"
                value={endDate}
            ...
            <div
              ...
              className={` animate center size-auto px-1 font-medium rounded-md cursor-pointer  ${
                activeFilter === "today"
          ...
          >
            Download Excel{" "}
            <i className="fa-regular fa-file-excel text-2xl text-green-500"></i>
          </div>
        </div>
        <div className="mt-5">
          <div className="patient-header flex border-b border-gray-200 justify-between items-center px-2 py-2 font-medium">
            <div className="w-[30%]">
              Patient Name{" "}
              <i className={`fa-solid fa-arrow-up text-xs text-gray-300 `}></i>{" "}
              <i
                className={`fa-solid fa-arrow-down  text-xs text-gray-300`}
              ></i>
            </div>
            <div className="w-[12%]">
              Gender{" "}
                ...
                {patientsToRender?.length > 0 ? (
                  patientsToRender.map((patient, index) => (
                    <motion.div
                      ...
                      className="font-medium patient-row flex border-b border-gray-100 justify-between items-center px-2 py-2 hover:scale-[1.001] hover:bg-gray-100 animate cursor-pointer rounded-md"
                    >
                      <div
                        className={` w-[86%] flex justify-between ${
                      ...
                      >
                        <div className="w-[30%] flex gap-1 items-center">
                          <img
                            src={patient?.image}
                            alt=""
                            className="bg-sky-400 min-w-8 size-8 rounded-full"
                          />
                        ...
                        <div className="w-[27%] hidden sm:block">
                          {patient?.complaint}
                        </div>
                      ...
                      <div
                        className={`w-[13%] flex justify-center items-center space-x-2 `}
                      >
                          ...
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <div
                                  onClick={() => {
                                    handleCopyPatientId(patient._id);
                                  }}
                                  className="px-2 py-1 hover:bg-gray-300 rounded-full min-w-8 size-8 animate flex items-center"
                                >
                                  <span>
                                    <LiaCopySolid className="text-blue-500 hover:text-blue-900" />
                                  </span>
                                </div>
                              </TooltipTrigger>
                              <TooltipContent>
                                <p className="size-full px-2 py-1 bg-gray-200 center rounded-md text-sm font-normal">
                                  Copy Patient ID
                                </p>
                              </TooltipContent>
                            </Tooltip>
                        ...
                        <button
                          disabled={selectedPatientId === patient?._id}
                        ...
                        <button
                          onClick={() => {
                            handleUpdateActive(patient?._id);
                          }}
                          className="rounded text-sm font-n h-7 min-w-20 text-gray-100"
                        >
                          {patient.active === false ? (
                            <div className="bg-green-400 center size-full rounded hover:bg-green-500">
                              Activate
                            </div>
                          ) : (
                            <div className="bg-red-400 center size-full rounded hover:bg-red-500">
                              Deactivate
                            </div>
                          )}
                        </button>
          ...
          <div className="flex justify-between pr-6 py-2 mt-2 absolute w-full bottom-1">
            ...
            showTodayPatients ? (
              <div className="w-fit p-2 rounded-md">
                Showing {patientsToRender?.length}
              </div>
            ) : (
              <div className="w-fit p-2 rounded-md">
                Showing {indexOfFirstPatient + 1} to{" "}
                {Math.min(indexOfLastPatient, patients?.length)} of{" "}
                {patients?.length}
              </div>
            )}{" "}
            {searchResults?.length >= 1 ||
            ...
            showTodayPatients ? (
              ...
              <div className="flex gap-12 items-center">
                <div className="flex gap-6 items-center">
                  ...
                  />
                  <button
                    onClick={handleGoToPageNumber}
                    className="px-2 py-2 w-12 bg-gray-300 hover:bg-black hover:text-white rounded-md"
                  >
                    Go
                  </button>
                </div>
                <div className="flex gap-2 rounded-md bg-gray-200">
                  <button
                    ...
                    className="px-2 py-2 text-gray-500 hover:text-red-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 rounded-md"
                  >
                    Previous
                  </button>
                  <button className="px-2 py-2 w-12 text-white bg-blue-500 hover:text-red-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 focus:rounded-md">
                    {currentPage}
                  </button>
                  <button
                    disabled={
          ...
          <AlertWrapper isOpen={openJobDoneAlert}>
            ...
            >
              <JobDoneAlert
                ...
                icon={
                  <i className="fa-regular fa-face-frown-open fa-bounce text-white pt-4"></i>
                }
              />
            </motion.div>
          </AlertWrapper>
