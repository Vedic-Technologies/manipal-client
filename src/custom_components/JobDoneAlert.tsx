...
const JobDoneAlert = ({ height, width, bgColor, icon,textColor, message, isOpen, OnCancel, isCancelButton, boxShadow }) => {
  ...
  return (
    <div
      className={`center inset-0 z-10 flex justify-center items-center  ${
        isOpen ? "" : "hidden"
      }`}
    >
      <div
      ...
      >
        ...
        <div className={`${isCancelButton} absolute top-1 right-0 `}>
          <button onClick={OnCancel} className={`center size-8 text-2xl rounded-full `}>
            <i className="fas fa-times  text-black cursor-pointer "></i>  
          </button>
        </div>
      </div>
    </div>
